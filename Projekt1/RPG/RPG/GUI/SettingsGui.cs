...
{
    public class SettingsGui : GuiGroup,IRegisterEvent
    {
        ...
        public void TouchedMulitiple(Vector2D position)
        {
            for (int i = 0; i < _sliders.Count; i++)
            {
                ...
                {
                    case 0:
                        _sliders[i].Touched(position);
                        Program.soundVolume = _sliders[i].Value;
                        break;
                    case 1:
                        _sliders[i].Touched(position);
                        MusicManager.GetInstance().ChangeVolume(_sliders[i].Value);
                        Program.musicVolume = _sliders[i].Value;
                        break;
                    default: return;
                }
        ...
        public void TouchedOnce(Vector2D position)
        {
            ...
            for (int i = 0; i < _sliders.Count; i++)
            {
                ...
                {
                    case 0:
                        if (_sliders[i].Touched(position))
                            OnPlay("menuselect");
                        break;
                    case 1:
                        if (_sliders[i].Touched(position))
            ...
            for (int i = 0; i < _checkboxes.Count; i++)
            {
                ...
                if (_checkboxes[i].Touched(position))
                    OnPlay("menuselect");
