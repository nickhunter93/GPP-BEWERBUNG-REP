...
namespace ConsoleApp2
{
    public class RectangleCollider : Component, ICollider
    {
        ...
        public RectangleCollider(Vector2D size, bool isStatic, bool isTrigger) : this()
        {
            Size = size;
            Static = isStatic;
            Trigger = isTrigger;
        ...
        public RectangleCollider(Vector2D size, bool isStatic, bool isTrigger, int activeTimer) : this(size,isStatic,isTrigger)
        {
        ...
        public void RecalculatePoints()
        {
            ...
            if(Parent != null)
                ...
                {
                    Points[i] = Points[i].Rotate(Math.PI * Parent.transform.WorldRotation / 180);
                }
        }
        ...
        {
            if (Static && collider.Static || !Active || _activeTimer > 0)
                return false;

            Vector2D mtd = Vector2D.Zero();
            List<Vector2D> axes = new List<Vector2D>();

            Position = Parent.transform.WorldPosition;
            collider.Position = collider.Parent.transform.WorldPosition;
        ...
        {
            if (Static && collider.Static || !Active)
                return false;
            ...
            /*if(Parent.ObjectName == "Weapon")
            {
                MyConsole.Out(Position + "");
            }

            if (collider.Parent.ObjectName == "Enemy")
            {
                MyConsole.Out(collider.Parent.transform.WorldPosition + "");
            }*/
            ...
            // Offset berechnen
            voffset = this.Position - collider.Parent.transform.WorldPosition;
            // A - alle Projektionsgeraden ermitteln und projezieren
            for (i = 0; i < this.Points.Length; i++)
            {
                l = i + 1;
